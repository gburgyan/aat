...
func TestCleanupStack_FILOOrder(t *testing.T) {
	...
	results := NewEngine(g, registry, NewExecutorRouter(executor, &adapter.EnvironmentConfig{})).runCleanupStack(context.Background(), stack, NewRunState())
...
func TestCleanupStack_Empty(t *testing.T) {
	stack := &CleanupStack{}
	results := (&Engine{}).runCleanupStack(context.Background(), stack, nil)
